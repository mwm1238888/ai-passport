/* ui_pixel.c — pixel-art theme for FoloToy AI Passport (upstream baseline).
 * Adapted for Work Companion: added screen_create_opts for CJK title font. */
#include "ui_pixel.h"
...
static void add_grass(lv_obj_t *scr)
{
...
static void add_title_plate(lv_obj_t *scr, const char *title,
...
{
    ui_pixel_scr_opts_t opts = { .title = title, .title_font = NULL, .bg = 0 };
    return ui_pixel_screen_create_opts(&opts);
}
...
lv_obj_t *ui_pixel_screen_create_opts(const ui_pixel_scr_opts_t *opts)
{
    ...
    lv_obj_set_style_bg_color(scr, lv_color_hex(opts->bg ? opts->bg : UI_SKY), 0);
    lv_obj_set_style_border_width(scr, 0, 0);
    lv_obj_set_style_pad_all(scr, 0, 0);
    add_cloud(scr, 188, 8);
    add_grass(scr);
    add_title_plate(scr, opts->title,
                    opts->title_font ? opts->title_font : &lv_font_montserrat_20);
    return scr;
}
